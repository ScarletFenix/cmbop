...
4. The job claims a `queued` row (`queued` → `sending`) or continues an
   ...
   **sent** (partial success) instead of overwriting that to failed.
   ...
   backlog is not a lost job). A matching row in `failed_jobs` still
   blocks reclaim (Email Center retry would double) but must **not**
   block expire — that job already died, and treating it as in-flight
