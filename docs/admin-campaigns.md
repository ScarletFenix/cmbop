...
4. The job claims a `queued` row (`queued` → `sending`) or continues an
   ...
   **sent** (partial success) instead of overwriting that to failed.
   ...
   Heal must use `latestDeliveredForCampaignUser()`, not only the attached id — an attached failed or leftover pending log FK must not beat a delivered sibling for the same campaign and user, including a historical `audience_campaign|{email}|AudienceCampaignMail` row (that marked a real send failed and a later compose doubled it). Heal also repairs a leftover `failed` recipient that already has that delivered sibling. Heal runs again after pending-log expire so a leftover pending FK closed in this pass can sync the same recover.
