@php
    ...
    $showBonus = ! $walletUnavailable && $walletBonus > 0.009;
@endphp
<div class="dash-wallet-strip">
    <div class="dw-item">
        <span class="dw-label">Spendable</span>
        <div class="dw-value">{{ $walletUnavailable ? '—' : '€'.number_format((float) ($wallet['spendable'] ?? 0), 2) }}</div>
    </div>
    @if($showBonus)
        <div class="dw-item">
    ...
    <div class="dw-item d-flex align-items-center">
        <a href="{{ route('advertiser.add-funds') }}" class="btn btn-sm btn-primary">
            @if(! $walletUnavailable && !empty($budgetStatus['low_balance']))
                Top up — low balance
            @else
                Add funds
            @endif
    ...
    @elseif(! $walletUnavailable && !empty($budgetStatus['monthly_limit']))
