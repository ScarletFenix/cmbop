...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-md-12">
            ...
            <p class="text-muted mb-0">
                View your balances and transfer funds from Advertiser to Publisher wallet.
            </p>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <!-- <div class="col-md-8 mb-4">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white fw-semibold">
                    <i class="fa fa-exchange-alt me-2 text-success"></i> 
                    Publisher Balance
                </div>
                <div class="card-body text-center">
                    <h2 class="mb-0" id="publisherBalance" style="color: #f59e0b;">€{{ number_format($publisherBalance, 2) }}</h2>
                    <p class="text-muted small mt-2">Your Publisher wallet balance</p>
                </div>
            </div>
        </div> -->
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white fw-semibold">
            <i class="fa fa-arrow-right-arrow-left me-2 text-info"></i> Transfer to Publisher Wallet
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <div class="mb-4 p-3 bg-light rounded">
                        <div class="row align-items-center text-center">
                            <!-- Left Side: Advertiser Balance (After Transfer) -->
                            <div class="col-12 col-md-5 mb-3 mb-md-0">
                                ...
                                <div class="mt-2">
                                    ...
                                    <span class="fw-bold text-primary fs-5" id="newAdvertiserBalance">€{{ number_format($advertiserBalance, 2) }}</span>
                                </div>
                            </div>

                            <!-- Arrow Icon fa exchange-alt -->
                            <div class="col-12 col-md-2 mb-3 mb-md-0">
                                <i class="fa fa-exchange-alt fa-2x text-muted d-none d-md-inline"></i>
                                <i class="fa fa-arrow-down fa-2x text-muted d-inline d-md-none"></i>
                            </div>

                            <!-- Right Side: Publisher Balance (After Transfer) -->
                            <div class="col-12 col-md-5">
                                ...
                                <div class="mt-2">
                                    ...
                                    <i class="fa fa-equals text-muted mx-1"></i>
                                    <span class="fw-bold text-success fs-5" id="newPublisherBalance">€{{ number_format($publisherBalance, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    $('#transferBtn').on('click', function() {
        ...
        Swal.fire({
            ...
            html: `<div class="text-start">
                <p><strong>From:</strong> Advertiser Wallet</p>
                <p><strong>To:</strong> Publisher Wallet</p>
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: baseUrl + '/advertiser/balance/transfer',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        amount: amount
                    ...
                    complete: function() {
                        $('#transferBtn').prop('disabled', false).html('<i class="fa fa-exchange-alt me-2"></i> Transfer to Publisher Wallet');
                        validateTransfer();
                    }
                });
            }
...
function updatePreview() {
    ...
    // Update current balances display
    $('#currentAdvertiserBalance').html('€' + advertiserBalance.toFixed(2));
    $('#currentPublisherBalance').html('€' + publisherBalance.toFixed(2));
    
    // Change color if negative
    if (newAdvertiserBalance < 0) {
        $('#newAdvertiserBalance').css('color', '#ef4444');
    } else {
        $('#newAdvertiserBalance').css('color', '#10b981');
    }
}
...
function loadTransferHistory(page = 1) {
    ...
    $.ajax({
        url: baseUrl + '/advertiser/balance/history?page=' + page,
        method: 'GET',
        success: function(response) {
            if (response.success) {
...
function renderTransferHistory(transfers) {
    ...
    transfers.forEach(function(transfer) {
        ...
        html += '<tr>' +
            ...
            '<td><span class="badge bg-primary">' + transfer.from_role.toUpperCase() + '</span></td>' +
            '<td><span class="badge bg-success">' + transfer.to_role.toUpperCase() + '</span></td>' +
